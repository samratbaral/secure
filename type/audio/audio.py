...
def audio_decode_result():
    if request.method == 'POST':
        ...
        if file:
            ...
            decryption = False
        result = request.form
        return render_template("decode-audio-result.html", result=result, decrypytedText=decrpytedText, file=file, decryption=decryption)
...
def encrypt(f_name, input_string):
    ...
    print(os.getcwd())
    path = str(os.getcwd()) + \
        "\modes\Audio\\ffmpeg-4.3.1-2020-10-01-full_build\\bin\\ffmpeg"
    print(path)
    ...
    sec_command = path + " -i tmp/%d.png -vcodec png modes/Audio/static/enc-audio.mp4 -y"
    print(sec_command)
...
def frame_extraction(audio):
    if not os.path.exists("./tmp"):
        os.makedirs("tmp")
    temp_folder = "./tmp"
    print("[INFO] tmp directory is created")

    audiotap = cv2.VideoCapture(audio)
    count = 0
    while True:
        success, image = audiotap.read()
        if not success:
            break
